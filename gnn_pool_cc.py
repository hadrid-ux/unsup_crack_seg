...
class GNNpool(nn.Module):
    ...
    def forward(self, data, A):
        ...
        # S_soft = F.gumbel_softmax(H, tau=1, hard=False)
        # S_hard = F.gumbel_softmax(H, tau=1, hard=True)
    ...
    def loss(self, A, S):
        ...
        cc_loss = -torch.sum(A * X) / comb(S.shape[0], 2)

        # orthogonality loss
        St_S = torch.matmul(S.t(), S)
        I_S = torch.eye(self.num_clusters, device=self.device)
        ortho_loss = (torch.norm(St_S / torch.norm(St_S) - I_S / torch.norm(I_S)) / self.num_clusters)

        # print("cc: {} ortho{}: ".format(cc_loss, ortho_loss))
        return cc_loss + ortho_loss
