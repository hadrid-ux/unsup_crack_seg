...
def GNN_seg(mode, epoch, K, pretrained_weights, in_dir, out_dir, save, cc, bs, log_bin, res, facet, layer,
            stride, device):
    """
    ...
    for filename in tqdm(os.listdir(in_dir)):
        ...
        if mode == 0:
            util.save_or_show([image, mask0, util.apply_seg_map(image, mask0, 0.7)], filename, save)
            continue
        ...
        if mode == 1:
            util.save_or_show([image, mask1, util.apply_seg_map(image, mask1, 0.7)], filename, save)
            continue
        ...
        util.save_or_show([image, mask2, util.apply_seg_map(image, mask2, 0.7)], filename, save)
...
if __name__ == '__main__':
    ...
    GNN_seg(mode, epochs, K, pretrained_weights, in_dir, out_dir, save, cc, bs, log_bin, res, facet, layer, stride,
            device)